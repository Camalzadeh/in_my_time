export async function POST(req: Request) {
  try {
    ...
    if (targetDates.length > MAX_DAYS) {
      return new Response(
        JSON.stringify({
          message: `Too many days selected. Maximum allowed is ${MAX_DAYS}.`,
        }),
        { status: 400 }
      );
    }
    ...
    if (slotDuration < MIN_SLOT) {
      return new Response(
        JSON.stringify({
          message: `Slot duration must be at least ${MIN_SLOT} minutes.`,
        }),
        { status: 400 }
      );
    }
    ...
    if (slotDuration > MAX_SLOT) {
      return new Response(
        JSON.stringify({
          message: `Slot duration cannot exceed ${MAX_SLOT} minutes.`,
        }),
        { status: 400 }
      );
    }
    ...
    return new Response(
      JSON.stringify({
        message: "Validation OK — you can now save to DB",
      }),
      { status: 200 }
    );
  } catch (err) {
